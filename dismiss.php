...
error_reporting(E_ALL ^ E_DEPRECATED);
	...
	echo '<form action="dismissexec.php" method="post" name="editadmin" onsubmit="return validateForm()">';
			...
			while($row = mysql_fetch_array($result))
  			...
  			echo'Name: '.'<input type="text" class="ed" name="uname" value="'. $row['userName'] .'" readonly>'; 
			   echo '<br>';
				...
				echo '<br>';
			  echo'Role: '.'<input type="text" class="ed" name="role" value="housemanager" readonly>';
			  echo '<br>';
			  echo'Member Id: '.'<input type="text" class="ed" name="userid" value="'. $row['user_id_fk'] .'" readonly>';
			  echo '<br>';
			  echo '<input name="" type="submit" value="Dismiss this hm" />';
  			}
	echo '</form>';
			}
			?>
